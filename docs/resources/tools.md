...
医療系での使い方は、[患者用ポータルで狙われやすい脆弱性](../web-security/patient-portal.md)を参照してほしい。
...
| 対象 | 構築方法 |
|---|---|
| OpenEMR | 公式の Docker 構成を利用（[詳細](../oss-vulnerabilities/ehr-systems.md)） |
| Orthanc | `docker run jodogne/orthanc-plugins`（[詳細](../oss-vulnerabilities/imaging-pacs.md)） |
