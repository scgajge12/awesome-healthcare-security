...
**事実**：警察庁が公表した令和 7 年の確定値では、特殊詐欺の認知件数は 2 万 7832 件、被害額は 1423.1 億円である。
うち還付金詐欺は 3179 件、61.9 億円で、還付金詐欺では接触手段の 9 割以上が電話である。
...
**分析**：ここまでは、窃取したデータを現金に変える経路の話である。
国内の被害では、これとは別に、請求そのものを止めて恐喝の材料にする経路が働く。
同じ現物給付の構造が、こちらでは攻撃側に有利に作用する。
保険診療の収入は診療報酬に集中しているため、電子カルテと医事会計を同時に止めれば、保険者への請求も窓口での算定も塞がる。

**事実**：保険医療機関は、1 か月分の診療報酬明細書（レセプト）を診療月の翌月 10 日までに審査支払機関へ提出する。支払基金は、医療機関への振込を診療月の翌々月の原則 21 日までとしている（[請求命令 第 2 条第 1 項](https://laws.e-gov.go.jp/law/351M50000100036)、[社会保険診療報酬支払基金 診療報酬の審査・支払業務の流れ](https://www.ssk.or.jp/smph/shinryohoshu/gyomuflow/index.html)）。

**分析**：データの単価を積み上げる見方だけでは、国内の医療機関が狙われる理由を説明しきれない。
止めれば月単位の入金が止まるという性質のほうが、要求額の設定に直接効く。
仕組みと桁は[連鎖するランサムウェア攻撃](ransomware-chain.md#診療報酬の請求サイクル)に整理した。

### 攻撃者側の採算

**事実**：米司法省の公表によれば、LockBit は 120 か国以上で 2500 を超える被害者を出し、少なくとも 5 億ドルの身代金を得た。
...
**報道ベース**：米国では、患者の医用画像を含むデータを公開された Lehigh Valley Health Network が 6500 万ドルの集団訴訟和解に応じたと報じられている。
