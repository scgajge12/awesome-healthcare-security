...
<p align="center">
  <img src="../../../assets/org-vulnerability-taxonomy.svg" alt="組織の脆弱性の分類。人、プロセス、システムの三層に分かれ、システムの脆弱性の内側に既知、CVE がある、パッチがある、自組織に適用済みが入れ子になる" width="100%">
</p>
...
| 自組織に適用済み | 自組織の資産で解消されている。作業を誰が行うかは、組織の形によって変わる | 資産台帳、構成管理の記録 |
...
自組織が動かせるのは「パッチがある脆弱性」と「自組織に適用済み」の差だけである。
攻撃者に残された余地も、まずこの差に現れる。

**分析**：ただし、この差を誰の手で詰めるかは、組織によって違う。
病院では適用の可否も作業も納入ベンダの側にあり（[病院の場合](#病院の場合)）、製薬企業では自社の変更管理が可否を決める（[製薬企業の場合](#製薬企業の場合)）。
...
<p align="center">
  <img src="../../../assets/org-vulnerability-hospital.svg" alt="病院の組織的な脆弱性。人、プロセス、システムの三層それぞれに病院固有の項目を配置し、システムの層ではベンダが適用を認めた段階を挟んだ図" width="100%">
</p>

病院では、診療を止められないという制約と、システムの構築と運用を納入ベンダに委ねている構成が、三層すべてに効いてくる。
人の層では確認の手順が省かれ、プロセスの層では適用の窓が取れず、システムの層では適用の可否を自院だけでは決められない。
...
| **パッチ適用の窓が取れない** | 24 時間稼働であるうえ、適用の時期をベンダの動作検証と保守要員の日程に合わせることになる | 適用できない資産を一覧にし、分離と監視で補う。適用の期限を機器の重要度別に決め、ベンダとの保守契約に書く |
...
| **保守の範囲が確定していない** | 自院は事業者が保守していると考え、事業者は保守対象でないと考えていることがある | 対象機器とセキュリティアップデートの扱いを契約書で確認する。責任分界を文書に残す |
...
| 資産 | 図のどこに入るか | 打ち手 |
|---|---|---|
| **VPN 装置、リモートアクセス製品** | CVE があり、パッチもある。適用の遅れがそのまま露出になる | 版数の一覧を持ち、KEV 掲載の有無で適用の順序を決める。保守契約の対象かどうかを機器ごとに確認する |
| **患者向けの公開サービス** | 委託して構築した部分の欠陥は CVE を持たない | 認証と認可の欠陥は識別子では拾えないため、[診断](../../practice/pentest/)で確認する。改修の依頼先を契約で決めておく |
...
| **電子カルテ** | ベンダが提供する修正が、製品のバージョンに紐づく。適用の作業もベンダが行う | 適用の可否と時期をベンダと合意し、合意内容を記録に残す |
| **事務用の端末、グループウェア** | 電子カルテと切り離されていれば、パッチの公表から適用済みまでを自院の判断で動かせる | 更新の自動適用を既定にし、外した端末とその理由を台帳に残す |
...
限られた工数をどちらに置くかを決めるとき、図の内側（自院に適用済み）と外側（パッチがある）の差を先に埋めるほうが、経路の数を減らしやすいということである。

### 病院で適用を決めるのは誰か

**事実**：厚生労働省医政局医療情報担当参事官室は 2026 年 3 月 19 日、VPN 装置等のネットワーク機器についての確認事項を事務連絡で示した。
確認項目の最初は保守管理体制と責任分界の明確化であり、自院は事業者に保守されていると考える一方で事業者は保守対象でないと考えていることなどが原因で、機器の管理と設定が適切に行われず被害に遭う事例が多発しているとしている。
自院の VPN 装置等の外部接続機器について、セキュリティアップデートを含む保守契約が結ばれているかの再確認を求めている。
出典：[VPN 装置等のネットワーク機器におけるサイバーセキュリティ対策の徹底について（周知依頼）（PDF）](https://www.mhlw.go.jp/content/10808000/001679871.pdf)

**事実**：厚生労働省の令和 7 年度版チェックリストは、「医療機関確認用」と「事業者確認用」の二部からなる。
セキュリティパッチ（最新ファームウェアや更新プログラム）の適用は、両方に同じ番号（2-⑥）で置かれている。
立入検査で確認されるのは医療機関確認用である。
出典：[令和 7 年度版 医療機関におけるサイバーセキュリティ対策チェックリスト（PDF）](https://www.mhlw.go.jp/content/10808000/001703610.pdf)

**事実**：医療情報システムの安全管理に関するガイドラインは、第 7.0 版（2026 年 6 月）で保守委託機関編が加わり、概説編、経営管理編、企画管理編、システム運用編、保守委託機関編の 5 編になった。
保守を受託する事業者に向けた編が、医療機関向けの編とは別に置かれている。
出典：[厚生労働省 医療情報システムの安全管理に関するガイドライン 第 7.0 版](https://www.mhlw.go.jp/stf/shingi/0000516275_00006.html)

**事実**：医療機器のサイバーセキュリティの確保及び徹底に係る手引書（令和 3 年 12 月 24 日付け通知の別添）は、セキュリティパッチ等のアップデートについて、医療機器としての機能の追加や変更がない場合には都度の薬事承認を受ける必要はないとしている。
市販後に脆弱性の修正を行う主体は製造販売業者であり、機器が接続される施設の IT 基盤についてのリスク管理とセキュリティ体制の構築が、医療機関の側の役割とされている。
出典：[医療機器のサイバーセキュリティの確保及び徹底に係る手引書について（厚生労働省）](https://www.mhlw.go.jp/web/t_doc?dataId=00tc6374&dataType=1&pageNo=1)

**事実**：日本医師会総合政策研究機構が 2021 年 5 月に公表した実態調査（病院と診療所あわせて 2,989 施設が回答、回収率 30.4%）では、情報システムの専任の担当部門があるのは 2 割強で、3 分の 2 弱は兼務の担当者または院長自身が管理していた。
院内のネットワーク構成図を保有し、計画的に見直しているのは 5.7% だった。
出典：[病院・診療所のサイバーセキュリティ（日本医師会総合政策研究機構）](https://www.jmari.med.or.jp/result/working/post-233/)

**分析**：病院での「適用済み」は、自院が作業した結果ではなく、ベンダが自院の構成で検証して適用し、その結果を自院が受け取った状態を指すことが多い。
自院の判断だけで当てられるのは、電子カルテや医療機器と切り離された事務用の端末など、保守契約の外にある範囲に限られる。
図で「パッチがある脆弱性」と「自院に適用済み」の間に「ベンダが適用を認めた」を挟んでいるのは、この構成を写したためである。

**分析**：適用の作業が契約の上にある以上、契約の境目に空白が生まれる。
自院は保守されていると考え、ベンダは対象外と考えていれば、その機器は誰にも当てられないまま「パッチがある脆弱性」の側に残る。
行政の事務連絡が、技術的な設定より先に責任分界の確認を置いているのは、この空白が経路になっているためである。

**分析**：この構成では、病院に残る手段は契約と日程と記録の三つになる。
保守契約に適用の期限と報告の形を書き、停止できる日程を診療側と調整して確保し、依頼と結果を自院の記録に残す。
依頼したまま適用されていない資産は、自院の台帳の上では「パッチがある脆弱性」にとどまる。
この件数を数えられるかどうかで、ベンダとの交渉に使える材料の有無が決まる。
...
その結果、修正が公表されてから自社に適用されるまでの期間が長くなる。
病院と違い、適用の可否を決めるのは自社の変更管理であり、作業を自社で行うかベンダに任せるかも自社で選べる。
遅れは、決める権限がないためではなく、決めるまでの手続きに時間がかかるために生じる。
...
**事実**：厚生労働省は、GMP 省令の対象となる医薬品と医薬部外品の製造販売業者等に向けて、コンピュータ化システム適正管理ガイドラインを示している（平成 22 年 10 月 21 日付け薬食監麻発第 1021011 号）。
運用管理業務の変更管理として、変更が計算機化システムに与える影響を評価し、その結果に基づいて措置を実施することを求めている。
出典：[医薬品・医薬部外品製造販売業者等におけるコンピュータ化システム適正管理ガイドラインについて（厚生労働省）](https://www.mhlw.go.jp/web/t_doc?dataId=00tb6573&dataType=1&pageNo=1)
...
| 適用を決める主体 | 納入ベンダ。自院で当てられるのは事務系の一部 | 自社の変更管理。作業の委託先は選べる |
| パッチが適用済みに至らない主因 | 提供されない、止められない、ベンダの検証と作業を待つ | 検証と承認に時間がかかる |
| 番号で数えられない部分 | 設定の露出、委託先の接続 | 委託先の環境、記録の完全性 |

**分析**：どちらも、システムの層で適用済みの枠を広げにくい。
病院ではメーカーが修正を出さないことと、ベンダの検証と作業の順番を待つことが、製薬企業では自社の検証が終わらないことが、それぞれ枠を押し広げにくくしている。
制約の形が違うため、補い方も分かれる。
病院は機器単位の分離と代替措置に加えて、契約と日程の確保が中心になり、製薬企業は適用計画と暫定の緩和策を組みにして進めることになる。
...
| システム | パッチがある脆弱性のうち未適用の件数、ベンダに依頼済みで未適用の件数、適用期限の超過件数、KEV 掲載分の残存件数 | 資産台帳、脆弱性管理の記録、保守の作業報告 |

進め方は、次の順で置くと差が見えやすい。

1. 資産台帳を先に作る。台帳にない資産は、既知の脆弱性の対象にすらならない。
2. 台帳と KEV を突き合わせ、「パッチがあるのに適用済みでない」件数を出す。
3. 適用できない資産について、理由（提供されない、止められない、ベンダの検証と作業を待っている、自社で検証中）を記録し、代替の緩和策を紐づける。
...
| 医療機関と事業者の役割分担 | [医療情報システムの安全管理に関するガイドライン 第 7.0 版](https://www.mhlw.go.jp/stf/shingi/0000516275_00006.html)、[令和 7 年度版 チェックリスト（PDF）](https://www.mhlw.go.jp/content/10808000/001703610.pdf) |
| 保守契約と責任分界の確認 | [VPN 装置等のネットワーク機器におけるサイバーセキュリティ対策の徹底について（PDF）](https://www.mhlw.go.jp/content/10808000/001679871.pdf) |
| 医療機器の修正の主体 | [医療機器のサイバーセキュリティの確保及び徹底に係る手引書について](https://www.mhlw.go.jp/web/t_doc?dataId=00tc6374&dataType=1&pageNo=1) |
| 医療機関の情報システム管理体制 | [病院・診療所のサイバーセキュリティ（日本医師会総合政策研究機構、2021 年）](https://www.jmari.med.or.jp/result/working/post-233/) |
| 国内の計算機化システムの要件 | [コンピュータ化システム適正管理ガイドライン](https://www.mhlw.go.jp/web/t_doc?dataId=00tb6573&dataType=1&pageNo=1) |
