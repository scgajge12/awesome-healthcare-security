...
進行中の年である。
本ページの収録は 2026 年 8 月時点で当事者が公表した事案に限る。
...
| [JP-2026-03](#JP-2026-03) | 2026-04 | 市立奈良病院（奈良市） | 不正アクセス（疑い） | 未公表 | 救急受入を停止、外来を制限 | 電子カルテほか **約 3 日**（4 月 21 日深夜から 24 日朝） | 当事者公表 |
...
### JP-2026-03 市立奈良病院（2026 年 4 月）

**事実**

- 発生時期：2026 年 4 月 21 日深夜。4 月 22 日に第 1 報、4 月 23 日に第 2 報を公表した。
- 対象組織：奈良市の市立奈良病院。
- 攻撃種別：不正アクセスの疑い。ネットワーク監視装置がサイバー攻撃と思われる異常な通信を検知した。
- 初期侵入経路：未公表。
- 発覚の経緯：ネットワーク監視装置による検知。検知後、電子カルテシステムに関係するサーバをネットワークから隔離した。
- 侵害範囲：電子カルテ、眼科、生理検査の **3 システム**。個人情報の漏えい、データの破損、ウイルス感染はいずれも確認されていない。
- 診療への影響：4 月 22 日に外来診療を制限し、同日 3 時に救急受入を停止した。処方と処置を手書きとし、二重チェック体制を敷いた。
- 業務停止期間：4 月 23 日 22 時に病棟の電子カルテを部分的に再開し、4 月 24 日 8 時 30 分に外来診療、入院診療、救急受入を含む通常診療を再開した。**約 3 日**である。
- 代替運用：処方と処置の手書き運用。患者にはお薬手帳の持参を求め、受付、診療、会計に時間がかかることを周知した。
- 公表された対策：国、県、警察および専門ベンダと連携し、全端末の安全性を確認したうえで段階的に復旧した。

**分析**

- 起点はネットワーク監視装置の検知である。同じ年の[武蔵小杉病院](#JP-2026-01)（端末の動作不良）や、[半田病院](2021-timeline.md#JP-2021-01)（プリンタからの犯行声明の印字）と比べると、覚知の手段が現場の異常ではなく監視である。検知から隔離までが同じ夜のうちに行われ、暗号化と情報の持ち出しのいずれも確認されないまま収束した。
- 検知が早いほど診療の停止は前倒しになる。この事案では、疑いの段階で救急を止めている。止める判断を早く下せるかどうかは、誰が権限を持つかを事前に決めてあるかで決まる（[サイバー BCP](../../../response/bcp-cyber.md)）。
- 停止した 3 システムのうち 2 つは部門システム（眼科、生理検査）である。電子カルテだけを見た復旧計画では、外来を再開できる状態にならない。何をどの順で戻すかは、診療の再開単位で決める必要がある。
- 「被害は確認していない」は「被害がなかった」とは異なる。確認できる範囲はログの保存期間と対象範囲に依存する（[ログと監視の設計](../../../technology/logging.md)）。

**一次情報**

- [市立奈良病院へのサイバー攻撃の疑いによる診療体制の一部制限と現状について（第 2 報）](https://www.city.nara.lg.jp/soshiki/93/264856.html)（奈良市、2026 年 4 月 23 日）
- [サイバー攻撃で電子カルテ停止、外来診療は再開 - 市立奈良病院](https://www.security-next.com/183764)（Security NEXT による当事者公表の報道）
