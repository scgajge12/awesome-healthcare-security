...
被害事例の側では、国内で 9 月 20 日時点までに 16 件のサイバー攻撃と 24 件のその他の事案を、海外で 99 件のサイバー攻撃と 5 件のその他の事案を収録した。
...
[GL-2026-95 Boston Scientific](../global/2026-timeline.md#GL-2026-95) は 8 月 25 日にオンプレミス基盤を侵害され、製造、受注、出荷が約 2 週間止まった。
この事案では、供給の停止が患者の状態にそのまま現れている。
植込み済みの機器の動作と既存の遠隔モニタリングには影響がない一方、新たにペースメーカや除細動器を入れた患者は、宅内の通信機を受け取れず、心臓のデータを外来へ送れない状態に置かれた。
...
### 流出した情報の性質が、氏名や番号と異なる事案が 2 件出た

**事実**：夏に公表された 2 件は、持ち出された情報の種類が他の事案と異なる。

[GL-2026-78 Baylor Genetics](../global/2026-timeline.md#GL-2026-78)（米）は、臨床遺伝子検査ラボである。
6 月 11 日から 17 日にネットワークの一部へ到達され、**281 万 878 人**分の検査情報と検査結果が対象となった。
検査業務そのものは中断していない。

[JP-2026-17 ロート製薬](../japan/2026-timeline.md#JP-2026-17)（国内）は、通信販売の顧客対応システムに保存されていた**顧客との通話音声データ**が持ち出された可能性を 9 月 15 日に公表した。
件数と対象者は調査中である。

**分析**：前者が扱うのは、遺伝情報に基づく検査結果である。
番号やパスワードと違い、書き換えて無効化することができず、影響は本人だけでなく血縁者にも及ぶ。
また、配列そのものを保管する場所を守っても、結果を記載した報告書が別の経路で外へ出れば、同じ影響が生じる（[ゲノムデータの保護](../../../technology/genomics.md)）。

後者が扱うのは音声である。
一般用医薬品の問い合わせでは、購入者が症状、服用中の薬、体質を口頭で話す。
内容としては要配慮個人情報に当たりうるが、文字として検索できないため、どこに何年分あるかを把握していない組織が多い。
応対品質の確認のために残される録音には、診療録のような保存期間の定めがない（[ログと監視の設計](../../../technology/logging.md)）。

2 件に共通するのは、情報の量ではなく性質が問題になる点である。
件数で並べると Baylor Genetics は本年の上位に入り、ロート製薬は件数すら出ていない。
だが、流出後に打てる手の有無で並べると順序は変わる。
保有する情報を棚卸しするときに、件数だけでなく「流出したら取り消せるか」を欄として持てるかが分かれ目になる。
...
### 診療システムの外側にある層が、8 月に二つの形で止まった

**事実**：8 月に公表された海外の 2 件は、いずれも電子カルテそのものを暗号化されていない。

[GL-2026-101 Health Sciences Centre、CancerCare Manitoba](../global/2026-timeline.md#GL-2026-101)（加）は、施設の保守管理系がランサムウェアに侵害された。
空調の中央監視、扉の入退室管理、エレベータ、職員 ID カードの発行が影響を受けた一方、臨床業務は中断していない。
空調は中央監視ではなく現地での監視により運転を続けた。

[GL-2026-100 JPS Health Network](../global/2026-timeline.md#GL-2026-100)（米）は、技術環境で不審な活動を検知した時点で自らネットワーク全体を落とした。
電子カルテ、患者ポータル、電話、検査、薬局が 11 日間使えなくなり、外傷、脳卒中、心筋梗塞を含む重症の救急搬送を他院へ転送した。
復旧の完了後も、本ページの作成時点（2026 年 9 月 20 日）で確認できる範囲では、攻撃種別も侵害の有無も公表されていない。

**分析**：前者は、病院の資産の数え方を問う事例である。
空調は手術室の陽圧と無菌製剤の調製を、入退室管理は手術部門と薬剤庫の区画を支えている。
資産台帳が医療情報システムだけを対象にしていると、この層は攻撃面として数えられない（[ネットワークの分離](../../../technology/segmentation.md)）。
中央監視が落ちても現地運転で凌げたのは設計の結果であり、集中管理を入れるときに「中央からの制御が失われた状態で何時間運転できるか」を要件として書けているかが分かれ目になる。

後者は、封じ込めの判断が診療の停止として現れた事例である。
暗号化も身代金の通知も確認されないまま、11 日間の停止と重症搬送の転送を引き受けている。
この判断を事案の最中に行うと、止めた側が後から説明を求められる。
止める条件と、判断の権限を誰が持つかを平時に決めておく必要がある（[サイバー BCP](../../../response/bcp-cyber.md)）。

原因が公表されないまま復旧だけが進んだ点は、国内の [JP-2026-S21 市立奈良病院](../japan/2026-timeline.md#JP-2026-S21) と対照的である。
奈良市の専門家会議は 4 か月を要して「外部からの攻撃ではない」と結論づけ、監視装置の自動更新と自動遮断の連鎖という機構まで報告書に書いた。
他施設が備える材料になるのは、停止の事実ではなく機構のほうである（[調査報告書から攻撃の流れを復元する](../report-reading.md)）。

### 国内は攻撃以外の経路が件数で上回っている

**分析**：国内の収録 40 件のうち、サイバー攻撃は 16 件、攻撃を伴わない事案は 24 件である。
病院系に限ると、攻撃 6 件に対してその他が 22 件で、3 倍を超える開きがある。
公表の慣行の差が件数に効いている点を差し引いても、患者情報が外へ出る経路は攻撃だけではない。

その他の 22 件のうち、院外の事業者が関わるものが 5 件ある。
...
**事実**：8 月には 5 件目が公表された。
京都府立医科大学では、附属病院で実習中の学生が実習報告書を写した写真を SNS へ投稿した（[国内の履歴](../japan/2026-timeline.md) の JP-2026-S23）。
大学は、患者個人を特定できる情報は写っていなかったとしたうえで、医療に従事する者として「極めて不適切」であるとしている。

**分析**：前の 4 件との違いは、投稿したのが職員ではなく学生である点と、公表された範囲では患者情報が含まれていない点である。
実習生は院内の規程の適用対象として整理されにくく、投稿の対象も患者の記録そのものではなく自分が書いた報告書だった。
撮影と投稿を「患者情報を扱う職員の問題」として設計すると、この経路は覆えない。
...
国内のその他の事案 24 件の類型別の内訳は、機器と記憶媒体の紛失、盗難が 7 件、誤送付、誤公開、操作ミスが 6 件、委託先の管理不備による漏えいが 5 件、内部不正が 3 件、サポート詐欺が 1 件、システム障害が 1 件、ドメインの失効が 1 件である。
...
進行中の年であり、2026 年 9 月 20 日時点までの集計である。
...
一方、本リポジトリが収録できるのは組織名とともに公表された事案に限られ、国内は 9 月時点で 40 件である。
桁が二つ違う。

本リポジトリが収録できた誤交付は、海老名こじろう耳鼻咽喉科の 1 件である（[国内の履歴](../japan/2026-timeline.md) の JP-2026-S24）。
廃棄予定の問診票を印刷用の裏紙に使い、患者へ渡す資料の裏面に別の患者の問診票が載った。
公表時点で確認できた対象は 1 名で、用紙が使われていた期間と対象の範囲は特定に至っていない。
統計の側で最多を占める類型が、事例としては 1 件しか拾えていない。
...
**注意**：2025 年 10 月 1 日から 11 月 12 日までの 43 日間、連邦政府が閉鎖され OCR のポータルへの追加が止まった。その影響で 2026 年は掲載が遅れており、6 月の時点でも 3 月分の届出が追加され続けていた。
