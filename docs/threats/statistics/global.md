...
> [!NOTE]
> **表記ルール**：**事実**（一次情報で確認できる内容。出典を併記する）、**報道ベース**（報道や第三者の集計のみで確認でき、原資料を直接確認できていない内容）、**分析**（筆者の解釈）を書き分ける。
...
|---|---|---|---|
| [Internet Crime Report](https://www.ic3.gov/AnnualReport/Reports/) | FBI IC3 | IC3 に寄せられた申告（被害者の自発的な届出） | 重要インフラ16分野の一つとして集計 |
...
ランサムウェアの申告は 3,600 件超、報告された損失は 3,200 万ドル超である。
...
<div align="center">
  <img src="../../../assets/ic3-critical-infrastructure-2025.svg" alt="IC3に2025年に寄せられた重要インフラ16分野別のランサムウェアとデータ侵害の申告件数。医療・公衆衛生分野がランサムウェア460件、データ侵害182件で最多" width="100%">
</div>
...
2 位の重要製造業を 105 件上回る。
重要インフラに属さない組織からのランサムウェア申告は別に 1,400 件超あり、その内訳は法務サービス 18%、建設請負 17%、エンジニアリングと建築 10% と続く。
...
**報道ベース**：2025 年に影響を受けた人数は約 1 億 3,850 万人で、1 日あたり 2 件を超える大規模侵害が報告された計算になる。
2024 年は約 2 億 9,200 万人で、Change Healthcare の 1 件（約 1 億 9,270 万人）が大部分を占める。
...
**報道ベース**：DBIR 2026 は全体で 31,000 件超のインシデントと 22,000 件超の侵害を分析している。
医療分野では 1,492 件のインシデントと 1,438 件の情報開示が確認された。
...
一方で、警察庁の統計が示す「調査費用 1,000 万円以上が半数超」という水準は、日本の医療機関にとっても現実的な桁である。

## EU ENISA

### ENISA Threat Landscape 2025

**事実**：ENISA は 2024 年 7 月から 2025 年 6 月までの 4,875 件のインシデントを分析した。
...
**事実**：Sophos は 17 か国の 3,400 組織を対象とした調査のうち、医療提供者 292 組織の回答を分析した。
...
**事実**：米国 IC3 の集計では、医療・公衆衛生は重要インフラ16分野でランサムウェア申告件数が最多である。
