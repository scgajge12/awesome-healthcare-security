...
> [!NOTE]
> **表記ルール**：**事実**（一次情報で確認できる内容。出典を併記する）、**報道ベース**（報道のみで確認でき、当事者の公表資料では裏付けが取れていない内容）、**分析**（筆者の解釈）を書き分ける。
...
<div align="center">
  <img src="../../../assets/npa-ransomware-trend.svg" alt="ランサムウェア被害報告件数の半期別推移。令和2年下半期21件から令和4年上半期114件まで急増し、その後は半期94件から116件で高止まりしている" width="100%">
</div>
...
<div align="center">
  <img src="../../../assets/npa-ransomware-industry.svg" alt="ランサムウェア被害の業種別内訳を令和3年から令和7年まで並べた表。製造業が一貫して首位で、医療・福祉は令和4年の20件で3位に入った以外は6位から7位、令和7年は上位に掲載されていない" width="100%">
</div>
...
**事実**：1,000 万円以上を要した組織は 46 件で、有効回答の 51.7% にあたる。
回答時点で復旧中の組織が 25 件ある。
...
検挙件数は 2025 年に 15,108 件と過去最多である。
...
1 件あたりの漏えい人数は 1,000 人以下が 93.6%、漏えいした情報の形態は紙媒体のみが 76.9% を占めた。
...
最新の[令和 8 年 5 月 29 日公表分](https://www.mhlw.go.jp/stf/newpage_73213.html)（[資料 7](https://www.mhlw.go.jp/content/10808000/001705951.pdf)）は、G-MIS の ID が付与された 8,102 施設を対象に 2026 年 3 月 9 日から 4 月 24 日に実施し、有効回答は 5,736 施設（70.8%）だった。
...
導入済みの病院では、導入費用は 1,000 万円から 3,000 万円が最頻値だった。
...
**事実**：事業所数で見ると、令和 3 年経済センサス活動調査では医療，福祉が 485,925 事業所、製造業が 412,682 事業所である（[結果の概要](https://www.stat.go.jp/data/e-census/2021/kekka/pdf/k_outline.pdf)）。
医療施設数は 179,645 施設（病院 8,060、一般診療所 105,207、歯科診療所 66,378。[令和 6 年医療施設動態調査](https://www.mhlw.go.jp/toukei/saikin/hw/iryosd/24/dl/11gaikyou06.pdf)）である。
...
復旧に 1 か月以上を要した組織は有効回答の 2 割を超え、調査費用が 1,000 万円以上に達した組織は半数を超えている。
