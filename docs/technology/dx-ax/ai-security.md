...
診療録や部門システムの文書を生成 AI の検索対象にするには、いったん索引（ベクタストアなど）へ写す。
写した先には、電子カルテ側の「担当患者だけが見える」という制御が付いてこない。
索引は文書とその埋め込みの集合であり、誰が誰の担当かという関係を既定では持たないためである。

```mermaid
flowchart LR
    subgraph SRC["元のシステム"]
        E["電子カルテ、部門システム<br>担当関係、診療科、病棟で<br>参照範囲が決まる"]
    end

    subgraph IDX["検索用の索引"]
        V["文書と埋め込み<br>参照範囲の属性は<br>写す設計にしない限り付かない"]
    end

    E -->|索引を作る| V
    V -->|類似度で引く| Q["利用者の問い合わせ"]
    Q --> M["LLM が要約して返す"]

    AC["元システムのアクセス制御"] -.->|効く| E
    AC -.->|効かない| V
```

**事実**：OWASP Top 10 for LLM Applications 2025 の LLM08 は、アクセス制御が不十分または不整合な場合に、機微な情報を含む埋め込みへの不正なアクセスが生じうるとしている。
複数の利用者区分やアプリケーションが同じベクタデータベースを共有する環境では、利用者間や問い合わせ間で文脈が漏れる危険があるとし、対策として、粒度の細かいアクセス制御と権限を認識するベクタストア、およびベクタデータベース内のデータセットを利用者の区分ごとに論理的かつアクセス上分割することを挙げている（[LLM08:2025 Vector and Embedding Weaknesses](https://genai.owasp.org/llmrisk/llm082025-vector-and-embedding-weaknesses/)）。

**抑え方**：索引に入れる文書ごとに、参照できる範囲（診療科、病棟、担当関係、患者本人）を属性として持たせ、検索の段でその属性を条件に加える。
それができない場合は、索引を診療科や用途の単位で分け、利用者が引ける索引を限る。
元システムの権限が変わったとき（異動、退職、担当の交代、同意の撤回）に、索引側へ反映する経路を決めておく。
索引の再構築でしか反映しない設計では、再構築の間隔がそのまま、権限を失った者が参照できる期間になる。

**検知**：検索の問い合わせと、返した文書の識別子を、利用者と対にして記録する。
条件にできるのは、担当関係のない患者の文書が返った件数と、短時間に多数の患者へまたがって返った検索である。
どちらも[患者用ポータルの脆弱性](../web-security/patient-portal.md)で扱った横断参照と同じ形をしており、記録がなければ事後に範囲を確定できない。

**分析**：この段は、モデル側の設定では塞がらない。
索引に写す時点で権限が落ちているため、モデルが何を返すかを制御する前に、検索が返しうる文書の範囲が決まっている。
導入審査でモデルとガードレールだけを見ると、この段が審査の対象から外れる。
...
診察のやり取りを録音し、文字起こしと要約を経て、診療録の下書きを作る使い方が広がっている。
この形では、これまで存在しなかった保存対象が三つ増える。
音声そのもの、文字起こし、生成された下書きである。

**抑え方**：三つのそれぞれについて、保存先、保存期間、削除の方法、アクセスできる者を決める。
音声は、要約を作った後に保持し続ける必要があるかを検討し、保持する場合は理由を記録する。
録音していることを患者へ知らせる方法と、断られたときの運用を決めておく。
診療録として確定させる前に、確定者が内容を確認する工程を必ず置く（[認証とアクセス管理](../identity.md)の記録の確定を参照）。

**検知**：どの診察が記録され、誰が音声と文字起こしにアクセスしたかを、記録として残す。
下書きと確定後の記録の差分を確認できる形にしておく。

**分析**：この用途に固有の論点は二つある。
第一に、要約の誤りは、情報の漏えいではなく記録の誤りとして患者に届く。
文字起こしの取り違えや、話されていない内容の混入が下書きに入りうるため、確定操作を省く運用にすると[完全性の問題](../../threats/integrity-attacks.md)になる。
第二に、録音は診療録とは別の法的性質を持ちうる。
同席者の音声が含まれること、録音に関する規律が国や地域によって異なることから、海外の事業者のサービスを使う場合は、適用される法域を確認する必要がある。
...
| LLM08 ベクタと埋め込みの弱点 | 検索対象の索引に、参照権限のない患者情報が混ざる（[索引に写した文書の参照範囲](#索引に写した文書の参照範囲)） |
