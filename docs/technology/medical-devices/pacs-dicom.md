...
使えるツールは [ツール](../../reference/resources/tools.md#dicom医用画像) に挙げている。

---

## 遠隔画像診断

画像を院外の読影医へ送る運用では、接点が三つ増える。
委託先の事業者、画像が通る経路、読影医が使う端末である。

**事実**：日本医学放射線学会は「遠隔画像診断に関するガイドライン 2023 第 2 版」を公表している。
また、保険診療における遠隔画像診断の管理について、令和 6 年 3 月 5 日付で「保険診療における遠隔画像診断に関する管理指針」を公表している。
指針は、遠隔画像診断に従事する医師について、放射線診断専門医としての一般的な教育に加えて、ネットワークの運用とセキュリティ対策に関する教育を受けることを求めている。
出典：[遠隔画像診断に関するガイドライン 2023 第 2 版](https://www.radiology.jp/guideline/20260304_1.html)、[保険診療における遠隔画像診断に関する管理指針（PDF）](https://www.radiology.jp/content/files/20240305_01.pdf)

| 接点 | 確認すること | 医療機関側で決められること |
|---|---|---|
| 委託先の事業者 | 保管場所、保存期間、再委託の有無、終了時の返却と削除 | 契約条項として定める |
| 経路 | 送信の方式、暗号化、送信元と宛先の限定 | 院内側の接続点を限定する |
| 読影医の端末 | 端末の管理主体、画像の端末内への保存の可否、認証方式 | 保存を許すかどうかを契約で定める |
| 送る画像の範囲 | 読影に不要な患者情報が含まれていないか | タグの扱いを送信前の工程に組み込む |

**分析**：この構成は、[ネットワークの分離](../segmentation.md)で扱う保守回線と同じ性質を持つ。
院内から見ると、外部の主体が院内の画像に到達する経路が常設される。
読影の質に関する管理と、経路の管理は別の担当になりやすいため、どちらの管理下にもない状態が残りやすい。
