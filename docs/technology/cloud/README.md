...
**分析**：厚生労働省ガイドラインは第 6.0 版（2023 年 5 月）で読み手の役割ごとに編を分ける構成へ再編され、第 7.0 版（2026 年 6 月）では保守委託機関編が加わって 5 編になった。
事業者向けガイドラインも第 2.0 版で対象事業者の定義が変わっている。
公開時点の版に対応したリファレンスを、そのまま現行版の適合証明として使うわけにはいかない。
版のずれはどのクラウドのリファレンスでも起こりうるため、参照する前に対応版を確認する必要がある（[国内のガイドラインと法規制](../../guidelines/japan.md#医療情報システムの安全管理に関するガイドライン)）。
...
- 医療機関向けクラウドサービス対応セキュリティリファレンス（三菱総合研究所）：クラウドサービスが 三省二ガイドラインの要求に適合するかを第三者として確認した資料。公開ページの所在が変わっているため、[三菱総合研究所](https://www.mri.co.jp/) のサイト内で表題を検索する。
