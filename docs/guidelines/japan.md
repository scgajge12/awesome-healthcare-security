...
それまで「ガイドラインに沿った対応が望ましい」という位置づけだったものが、法令上の義務に格上げされた点が重要な変化である。
...
| 所管 | ガイドライン | 対象 |
|---|---|---|
| 厚生労働省 | 医療情報システムの安全管理に関するガイドライン | 医療機関等 |
| 経済産業省, 総務省 | 医療情報を取り扱う情報システム, サービスの提供事業者における安全管理ガイドライン | 事業者 |

かつては総務省と経済産業省が別々のガイドラインを出しており、あわせて「三省四ガイドライン」と呼ばれていた。
2020 年に事業者向けが一本化され、現在の呼称になった。
医療機関とベンダが責任分界を議論するときは、この二つを突き合わせる形になる。

**参照先**：[経済産業省 医療情報を取り扱う情報システム、サービスの提供事業者における安全管理ガイドライン](https://www.meti.go.jp/policy/mono_info_service/healthcare/iryou/johoka/iryoujouhou_anzenkanri.html)
...
| 文書 | 内容 |
|---|---|
| 医療機器のサイバーセキュリティ導入に関する手引書 | IMDRF ガイダンスを踏まえた、国内向けの実装指針 |
| JIS T 81001-5-1 | 医療機器ソフトウェアのセキュアな開発ライフサイクル（IEC 81001-5-1 に対応） |
| JIS T 2304 | 医療機器ソフトウェアのライフサイクルプロセス（IEC 62304 に対応） |
| JIS T 14971 | 医療機器のリスクマネジメント（ISO 14971 に対応） |

**参照先**：[医薬品医療機器総合機構（PMDA）](https://www.pmda.go.jp/) ／ [厚生労働省 医療機器のサイバーセキュリティ](https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000179749_00004.html)
...
**参照先**：[医療、介護関係事業者における個人情報の適切な取扱いのためのガイダンス](https://www.ppc.go.jp/personalinfo/legal/)
...
| [一般社団法人医療 ISAC](https://m-isac.jp/) | 医療分野の脅威情報共有 |
...
| [医療機関向けセキュリティ教育支援ポータルサイト（MIST）](https://mist.mhlw.go.jp/) | 厚生労働省による研修コンテンツと、インシデント発生時の相談, 初動対応依頼の窓口 |
